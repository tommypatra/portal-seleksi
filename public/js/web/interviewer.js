...
function renderData(response) {
    ...
    if (response.data.length > 0) {
        $.each(response.data, function(index, dt) {
            ...
            const bgver = (dt.wawancara_status) ? 'success' : 'danger';
            const row = `<tr>
                        <td>${no++}</td>
                        <td>
                            <figure>
                        ...
                        <td><span class="badge text-bg-${bgver}">${dt.wawancara_mulai} sd ${dt.wawancara_selesai}</span></td>
                        <td>${dt.jumlah_pendaftar}</td>
                        <td>${dt.tms}</td>
                        <td>${dt.ms}</td>
                        <td>${dt.persen}%</td>
                        <td>
                            <a href="${base_url}/daftar-peserta-interview/${dt.id}" class="btn btn-primary" type="button"><i class="bi bi-person-vcard"></i></button>
                        </td>
                    </tr>`;
            dataList.append(row);
        });
